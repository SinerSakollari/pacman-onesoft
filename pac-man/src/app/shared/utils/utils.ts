import { map, scene, level, SPEED } from 'src/app/component/game/phaserGame';
import { Tile, tileType } from 'src/app/component/game/elements/tile.component';
import { directionEnum } from '../models/direction.model';

export class Utils {
  public static requestMovementInformation(elem: any): directionEnum {
    ...
    if (neighborTile.type === tileType.TELEPORT) {
      if (elem.player)
        elem.changeCurrentPosition(
          this.convertTilePosToXY(
            neighborTile.opositeTeleportPosition!,
            'pacman'
          )
        );
      else
        elem.changeCurrentPosition(
          this.convertTilePosToXY(neighborTile.opositeTeleportPosition!)
        );
    ...
    if (
      ...
      (neighborTile.type === tileType.DOOR &&
        elem.requestedDirection != 'NORTH')
    )
      return elem.actualDirection;
  ...
  public static findAlternativeWay(
    ...
    var dir: directionEnum = directionEnum.NORTH;
    if (nextWay == 'long') {
  ...
  public static convertTilePosToXY(
    { x, y }: { x: any; y: any },
    pacman?: string | null
  ) {
    if (pacman) {
