...
export class BlueGhost extends Enemy {
  ...
  private chase(): Tile {
    ...
    if (redPosition.x >= pacmanPosition.x) distanceX = -distanceX;

    if (redPosition.y >= pacmanPosition.y) distanceY = -distanceY;

    x = pacmanPosition.x + distanceX;
    y = pacmanPosition.y + distanceY;

    if (x >= map.MAP_WIDTH - 50) x = map.MAP_WIDTH - 50;
    else if (x <= 0) x = 100;

    if (y >= map.MAP_HEIGHT - 50) y = map.MAP_HEIGHT - 50;
    else if (y <= 0) y = 100;
