...
import { tweenMovement } from "src/app/shared/utils/animation";
...
export abstract class Enemy {
  ...
  constructor(position: Position, ghost: any) {
    this.ghost = ghost;
  ...
  private move() {
    ...
    this.ghost.anims.play(animationName, true);
  }

  private findRoute(): directionEnum | null {
  ...
  public sentToCage(enemySprite: any) {
    ...
    let image = scene.add.image(self.position.x, self.position.y, "eaten_ghost");
    tweenMovement(image, () => {
      image.destroy();
