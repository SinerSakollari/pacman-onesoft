import { map, ENEMY_SPAWN_TIME } from '../phaserGame';
  ...
  public setNextTile() {
    if (this.getCurrentTile().type == 'TELEPORT') {
      this.actualDirection =
        this.getCurrentTile().opositeTeleportPosition!.x == 0
          ? directionEnum.WEST
          : directionEnum.EAST;
      this.nextTile = map.getTile(
        this.getCurrentTile().opositeTeleportPosition!
      );
    } else
      this.nextTile = map.getNeighborTile(
        this.getCurrentTile(),
